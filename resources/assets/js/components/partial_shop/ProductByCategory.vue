		...
		<template v-else>
			<div class="col-md-4" v-for="(product, index) in products" :key="index">
				<div class="card">
					...
					<div class="card-body">
						<h6 class="text-center">{{  product.name }}</h6></h6>
						<div class="text-center">
							...
							<template v-else>
								<a :href="'/product/view/'+product.url" class="btn btn-dark ifg-btn mt-2" v-if="customer != 'No Email'">Choose an option</a>
							</template>
						</div>
	...
	export default {
		...
		methods: {
			getProductByCategory() {
				this.loading = true;
				axios.get('/api/product-by-category/'+this.category)
				.then((response) => {
					this.loading = false;
