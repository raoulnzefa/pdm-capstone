<template>
	<div class="row">
		<div class="col-md-12 col-sm-12">
			<h2 class="mt-4 mb-4">Voucher code</h2>
			<button class="btn btn-primary mb-4" @click="addVoucherCode"><i class="fa fa-plus"></i> Add voucher code</button>
            ...
            <div class="alert alert-danger" v-if="server_errors.length != 0">
		      ...
		      <div class="form-group">
		      	...
		      	<input type="text" class="form-control" tabindex="3" placeholder="Enter voucher discount"
		      	...
		      	<div v-if="$v.voucher.discount.$error">
                	...
                	<template v-if="$v.voucher.discount.numbersOnly">
                		<span class="error-feedback" v-if="!$v.voucher.discount.between">Please enter a discount percent between {{$v.voucher.discount.$params.between.min}} and {{$v.voucher.discount.$params.between.max}}</span>
                	</template>
                </div>
		      </div>
		    </b-modal>
	...
	export default {
		...
		data() {
			return {
				...
				voucherTypes: ['Subtotal discount', 'Free shipping'],
				ok_title: '',
				modal_title: '',
				isEdit: false,
				isSubmitted: false,
				...
				voucher: {
					...
					discount: {
						...
						required: requiredIf(function() {
							return (this.voucher.type === 'Subtotal discount') ? true : false
						})
					}
				}
			}
		...
		methods: {
			...
			addVoucherCode() {
				this.modal_title = 'Add voucher code';
			...
			saveVoucherCode(e) {
				...
				if (!this.$v.voucher.$invalid) {
					...
					if (!this.isEdit) {
						axios.post('/api/voucher-code/create', {
						...
						.then(response => {
							...
							if (response.data.success) {
								Swal('Voucher code has been created', '', 'success')
								.then((okay) => {
									if (okay) {
