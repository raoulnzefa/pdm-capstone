...
class PaymentController extends Controller
{
    ...
    {	
        $checkoutDetail = CheckoutDetail::where('id', $request->checkout_id)->first();
        
        $checkout_id = $request->checkout_id;

        $customer_id = $checkoutDetail->customer_id;
        $first_name = $checkoutDetail->first_name;
        $last_name = $checkoutDetail->last_name;
        $street = $checkoutDetail->street;
        $province = $checkoutDetail->province;
        $province_id = $checkoutDetail->province_id;
        $municipality = $checkoutDetail->municipal;
        $municipality_id = $checkoutDetail->municipal_id;
        $barangay = $checkoutDetail->barangay;
        $barangay_id = $checkoutDetail->barangay_id;
        $zip_code = $checkoutDetail->zip_code;
        $phone_number = $checkoutDetail->phone_no;
        $company = $checkoutDetail->company;
        $shipping_method = $checkoutDetail->shipping_method;
        $payment_method = $checkoutDetail->payment_method;
        $customer = Customer::where('id', $customer_id)->first();
        $shipping_fee = $checkoutDetail->shipping_fee;
        $subtotal = str_replace(',', '', session()->get('subtotal'));

        if (!is_null($checkoutDetail->voucher_code))
        {
            $discount = str_replace(',','',(float)$checkoutDetail->discount);
            $new_subtotal = $subtotal - $discount;

            if ($checkoutDetail->voucher_benefit == 'shipping')
            {
                $shipping_fee = 0;
            }
        }
        ...
        $cart_qty = $checkoutDetail->quantity;

        $cart_products = Cart::where('customer_id', $customer_id)->get();

        $total = str_replace(',', '', (float)$checkoutDetail->total);
        ...
        //db format
        $due_date = strftime("%Y-%m-%d", strtotime("+$d weekdays"));
        ...
        if ($payment_method === 'PayPal')
        {
            ...
            $details = new Details();
            $details->setSubtotal($new_subtotal)
           ...
           //dd($transaction);
            ...
            try {
              ...
              // Redirect the customer to $approvalUrl
                ...
                //     //When there's no intenet connection
                //     // echo 'Paypal-debug: '.$ex->getCode();

                //     // echo 'Paypal-debug: '.$ex->getData();
                //     //return redirect()->back()->with('paypal_error', $ex.' (config)');
                //     //return 'RETURN';
                // }
                // else
                // {
                //     // Session::put('error', 'Some error occur sorry for inconvient');
                //     // return Redirect::to('/payment/failed');

                //     //return 'Paypal: '.$ex->getMessage();
        ...
        else
        {
            ...
            try {
                ...
                //return 'payment 1st:'. $e->getMessage();
            ...
            CheckoutDetail::where('id', $checkout_id)->delete();
